...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist &artist) const
{
    ...
    Vertex *found = graph.at(artist.get_name());
    
    for (unsigned long int i = 0; i < found->neighbors.size(); i++)
    {
        Artist connected;
        connected = found->neighbors[i].neighbor;
                                            ...
                                            const Artist &dest) const
{
    ...
    Artist prev = get_predecessor(dest);
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * Depth First
 * purpose: COnduct a depth first search of the graph
 *
 * parameters: 2 artist (start and end for search) and the output stream
 * returns:    none
 */
void CollabGraph::depthfirst(Artist &start, Artist &end, ostream &out)
{
    if (is_marked(start) or is_marked(end))
    {
        no_path(start.get_name(), end.get_name(), out);
        return;
    }
    clear_metadata();
    stack<Artist> depth;
    ...
    {
        Artist top = depth.top();
        depth.pop();
        vector<Artist> connected = get_vertex_neighbors(top);
            for (unsigned long int i = 0; i < connected.size(); i++)
            {
                    if (not is_marked(connected[i]))
                    {
                        mark_vertex(connected[i]);
                        depth.push(connected[i]);
                        set_predecessor(connected[i], top);
                        if (connected[i] == end){
                            mark_vertex(end);
                            path_printer(report_path(start, end), out);
                            return;
                        }
                    }
            }
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * Breadth First
 * purpose: COnduct a breadth first search of the graph
 *
 * parameters: 2 artist (start and end for search) and the output stream
 * returns:    none
 */
void CollabGraph::breadthfirst(Artist &start, Artist &end, ostream &out)
{
    if (is_marked(start) or is_marked(end))
    {
        no_path(start.get_name(), end.get_name(), out);
    ...
    while (not bfs.empty())
    {
        Artist top = bfs.front();
        bfs.pop();
        vector<Artist> connected = get_vertex_neighbors(top);
            for (unsigned long int i = 0; i < connected.size(); i++)
            {
                Artist neigh = connected[i];
                    if (not is_marked(neigh))
                    {
                        mark_vertex(neigh);
                        bfs.push(neigh);
                        set_predecessor(neigh, top);
                        if (neigh == end){
                            mark_vertex(end);
                            path_printer(report_path(start, end), out);
                            clear_metadata();
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * no
 * purpose: does the not command 
 *
 * parameters: 2 artist (start and end for search) and the output stream
 *          a vector of artist which are not allowed to be used in the search
 * returns:    none
 */
void CollabGraph::no(Artist &start, Artist &end,vector<Artist> blocked, ostream &out)
{
    enforce_valid_vertex(start);
    enforce_valid_vertex(end);

    for (unsigned long int i = 0; i < blocked.size(); i++)
    {
        mark_vertex(blocked[i]);
    }
    breadthfirst(start, end, out);
    
}

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * no path
 * purpose: prints the message if there is not a path between artist
 *
 * parameters: 2 artist (start and end for search) and the output stream
 * returns:    none
 */
void CollabGraph::no_path(string start, string end, ostream &out)
{
    out << "A path does not exist between " << "\"";
    out << start << "\"" << " and " << "\"";
    out << end << "\".\n";
}
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * path printer
 * purpose: prints the path between two artist
 *
 * parameters: output stream and the stack of artist (from report path)
 * returns:    none
 */
void CollabGraph::path_printer(stack<Artist> path, ostream &out)
{
    unsigned int length = (path.size() - 1);
    for (unsigned long int i = 0; i < length; i++)
    {
        Artist here = path.top();
        path.pop();
        Artist future = path.top();
        out << "\"" << here.get_name() << "\"" << " collaborated with ";
        out << "\"" << future.get_name() << "\"" << " in ";
        out << "\"" << get_edge(here, future) << "\".\n";
    }
    out << "***" << endl;
}
